...
<?php if (!$crudLocked): ?><form method="post" class="bleep-layout" data-bleep-form>
    ...
    <section class="panel bleep-control-panel">
        ...
        <div class="bleep-standard <?= $bleepStandard['met'] === true ? 'met' : ($bleepStandard['met'] === false ? 'unmet' : '') ?>" data-bleep-standard><span>Indikator 80% Permenpora</span><strong data-bleep-standard-threshold><?= $bleepStandard['available'] ? e($bleepStandard['operator'] . ' ' . $bleepStandard['threshold'] . ' ml/kg/menit') : 'Belum tersedia untuk cabor ini' ?></strong><small data-bleep-standard-status><?= e($bleepStandard['label']) ?></small></div>
...
<section class="panel bleep-history-panel"><div class="panel-header"><div><p class="eyebrow">RIWAYAT MODUL</p><h2>Hasil Bleep Test Terbaru</h2></div><span class="count-badge"><?= e(count($history)) ?> data</span></div><div class="table-wrap"><table><thead><tr><th>No. Tes</th><th>Atlet</th><th>Cabor</th><th>Tanggal</th><th>Level.Shuttle</th><th>Jarak</th><th>VO2max</th><th>Kategori</th><th>Aksi</th></tr></thead><tbody><?php if (!$history): ?><tr><td colspan="9" class="empty-state">Belum ada hasil Bleep Test.</td></tr><?php endif; ?><?php foreach ($history as $row): ?><tr><td><span class="code-pill"><?= e($row['test_number']) ?></span></td><td><strong><?= e($row['athlete_name']) ?></strong><small class="cell-subtext"><?= e($row['officer_name']) ?></small></td><td><?= e($row['sport']) ?></td><td><?= e(format_date($row['test_date'])) ?></td><td><?= e($row['level']) ?>.<?= e($row['shuttle']) ?></td><td><?= e($row['distance_m']) ?> m</td><td><strong><?= e($row['vo2max']) ?></strong></td><td><?= e($row['category'] ?: '-') ?></td><td><div class="action-row"><?php if (!$crudLocked || (Auth::user()['role'] ?? '') === 'superadmin'): ?><a class="btn btn-small btn-light" href="<?= e(base_url('bleep-test.php?edit=' . $row['id'])) ?>">Edit</a><?php endif; ?><?php if ((Auth::user()['role'] ?? '') === 'superadmin'): ?><form method="post" data-confirm="Hapus permanen hasil Bleep Test <?= e($row['athlete_name']) ?>?"><?= csrf_field() ?><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= e($row['id']) ?>"><button class="btn btn-small btn-danger" type="submit">Hapus</button></form><?php endif; ?></div></td></tr><?php endforeach; ?></tbody></table></div></section>
