...
<section class="panel bleep-history-panel"><div class="panel-header"><div><p class="eyebrow">RIWAYAT MODUL</p><h2>Hasil Bleep Test Terbaru</h2></div><span class="count-badge"><?= e(count($history)) ?> data</span></div><div class="table-wrap"><table><thead><tr><th>No. Tes</th><th>Atlet</th><th>Cabor</th><th>Tanggal</th><th>Level.Shuttle</th><th>Jarak</th><th>VO2max</th><th>Kategori</th><th>Aksi</th></tr></thead><tbody><?php if (!$history): ?><tr><td colspan="9" class="empty-state">Belum ada hasil Bleep Test.</td></tr><?php endif; ?><?php foreach ($history as $row): ?><tr><td><span class="code-pill"><?= e($row['test_number']) ?></span></td><td><strong><?= e($row['athlete_name']) ?></strong><small class="cell-subtext"><?= e($row['officer_name']) ?></small></td><td><?= e($row['sport']) ?></td><td><?= e(format_date($row['test_date'])) ?></td><td><?= e($row['level']) ?>.<?= e($row['shuttle']) ?></td><td><?= e($row['distance_m']) ?> m</td><td><strong><?= e($row['vo2max']) ?></strong></td><td><?= e($row['category'] ?: '-') ?></td><td><a class="btn btn-small btn-light" href="<?= e(base_url('bleep-test.php?edit=' . $row['id'])) ?>">Edit</a></td></tr><?php endforeach; ?></tbody></table></div></section>
