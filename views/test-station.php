        ...
        <?php foreach ($rows as $row): ?>
            ...
            <article class="station-card <?= $row['result_value'] === null ? 'waiting' : 'done' ?>" data-station-card>
                ...
                <?php if (!crud_locked_for_current_user()): ?><form class="station-score-form" method="post" enctype="multipart/form-data">
                    ...
                    <div class="station-indicator <?= $row['indicator']['met'] === true ? 'met' : ($row['indicator']['met'] === false ? 'unmet' : '') ?>"><span>Indikator 80%</span><?php if ($row['indicator']['available']): ?><strong><?= e($row['indicator']['operator']) ?> <?= e(format_number_id($row['indicator']['threshold'])) ?> <?= e($rowDefinition['unit']) ?></strong><small><?= e($row['indicator']['label']) ?></small><?php else: ?><strong>Belum tersedia</strong><small>Tidak ada padanan standar untuk cabor/tes ini.</small><?php endif; ?></div>
