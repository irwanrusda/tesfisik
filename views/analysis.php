...
<section class="panel vo2max-analysis">
    ...
    <div class="vo2max-summary"><div><span>Rata-Rata</span><strong><?= e(format_number_id($vo2maxSummary['average'] ?? null)) ?></strong><small>ml/kg/menit</small></div><div><span>Terendah</span><strong><?= e(format_number_id($vo2maxSummary['minimum'] ?? null)) ?></strong><small>ml/kg/menit</small></div><div><span>Tertinggi</span><strong><?= e(format_number_id($vo2maxSummary['maximum'] ?? null)) ?></strong><small>ml/kg/menit</small></div><p>VO2max dihitung otomatis dari level dan shuttle terakhir Bleep Test menggunakan tabel VO2max Lari Multi Tahap.</p></div>
</section>
...
<section class="panel bleep-top-panel">
    ...
    <div class="table-wrap"><table><thead><tr><th>Peringkat</th><th>Atlet</th><th>Cabor</th><th>Level.Shuttle</th><th>Jarak</th><th>VO2max</th><th>Tanggal</th></tr></thead><tbody><?php if (!$bleepTopAthletes): ?><tr><td colspan="7" class="empty-state">Belum ada hasil Bleep Test.</td></tr><?php endif; ?><?php foreach ($bleepTopAthletes as $index => $row): ?><tr><td><span class="sport-rank"><?= e($index + 1) ?></span></td><td><strong><?= e($row['athlete_name']) ?></strong></td><td><?= e($row['sport']) ?></td><td><?= e($row['level']) ?>.<?= e($row['shuttle']) ?></td><td><?= e(format_number_id($row['distance_m'], 0)) ?> m</td><td><strong><?= e(format_number_id($row['vo2max'], 1)) ?></strong></td><td><?= e(format_date($row['test_date'])) ?></td></tr><?php endforeach; ?></tbody></table></div>
</section>
...
<section class="panel ranking-panel">
    ...
    <div data-ranking-results>
        ...
        <div class="table-wrap"><table class="ranking-table"><thead><tr><th>Peringkat</th><th>Atlet</th><th>Cabor</th><th>L/P</th><th>Nilai</th><th>Indikator 80%</th><th>Tanggal</th><th>Petugas</th></tr></thead><tbody>
        <?php if (!$rankingRows): ?><tr><td colspan="8" class="empty-state">Belum ada nilai untuk komponen ini.</td></tr><?php endif; ?>
        <?php foreach ($rankingRows as $index => $row): ?><tr><td><span class="ranking-number <?= $index < 3 ? 'top' : '' ?>"><?= e($index + 1) ?></span></td><td><strong><?= e($row['athlete_name']) ?></strong></td><td><?= e($row['sport']) ?></td><td><?= e($row['gender']) ?></td><td><strong class="ranking-score"><?= e(format_number_id($row['result_value'])) ?></strong> <small><?= e($row['unit']) ?></small></td><td><span class="indicator-tag <?= $row['indicator']['met'] === true ? 'met' : ($row['indicator']['met'] === false ? 'unmet' : '') ?>"><?= $row['indicator']['available'] ? e($row['indicator']['operator'] . ' ' . format_number_id($row['indicator']['threshold']) . ' · ' . $row['indicator']['label']) : 'Belum tersedia' ?></span></td><td><?= e(format_date($row['test_date'])) ?></td><td><?= e($row['officer_name']) ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    </div>
</section>
