...
<section class="panel ranking-panel">
    ...
    <div data-ranking-results>
        ...
        <?php foreach ($rankingRows as $index => $row): ?><tr><td><span class="ranking-number <?= $index < 3 ? 'top' : '' ?>"><?= e($index + 1) ?></span></td><td><strong><?= e($row['athlete_name']) ?></strong></td><td><?= e($row['sport']) ?></td><td><?= e($row['gender']) ?></td><td><strong class="ranking-score"><?= e($row['result_value']) ?></strong> <small><?= e($row['unit']) ?></small></td><td><span class="indicator-tag <?= $row['indicator']['met'] === true ? 'met' : ($row['indicator']['met'] === false ? 'unmet' : '') ?>"><?= $row['indicator']['available'] ? e($row['indicator']['operator'] . ' ' . $row['indicator']['threshold'] . ' · ' . $row['indicator']['label']) : 'Belum tersedia' ?></span></td><td><?= e(format_date($row['test_date'])) ?></td><td><?= e($row['officer_name']) ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    </div>
</section>
