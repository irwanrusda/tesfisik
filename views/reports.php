<div class="page-heading">
    ...
    <?php if (in_array(Auth::user()['role'] ?? '', ['superadmin', 'panitia', 'input'], true) && !crud_locked_for_current_user()): ?><a class="btn btn-primary" href="<?= e(base_url('test-create.php')) ?>">＋ Daftarkan Atlet Tes</a><?php endif; ?>
</div>
...
<section class="panel">
    ...
    <?php foreach ($tests as $item): ?><tr>
        <?php $ageText = $item['birth_date'] ? calculate_age($item['birth_date'], $item['test_date']) . ' tahun' : 'Usia belum diisi'; ?>
        <td><span class="code-pill"><?= e($item['test_number']) ?></span></td><td><strong><?= e($item['athlete_name']) ?></strong><small class="cell-subtext"><?= e($ageText) ?> · Input: <?= e($item['creator_name']) ?></small></td><td><?= e($item['sport']) ?></td><td><?= e($item['gender']) ?></td><td><?= e(format_date($item['test_date'])) ?></td><td><?= e($item['bmi'] ?: '-') ?></td>
